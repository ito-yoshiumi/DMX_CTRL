...
### テスト5: 参加者5音の録音＆ミックス再生

**確認内容:**
- 各録音エントリで所定秒数の録音が開始・停止する
- 最大5件まで録音が蓄積される
- フィナーレで録音とお手本のミックスが再生される

**手順:**
1. `ScenarioRunner`に`ParticipantRecordingManager`がアタッチされていることを確認
2. `ParticipantRecordingManager.audioInputManager`に`AudioInputManager`を設定
3. シナリオを実行し、プロンプトに従って5回入力する
4. コンソールに「参加者録音開始」ログが5回表示されることを確認
5. フィナーレの直前で「録音ミックスを再生します。」ログが出力され、AudioSourceから音声が再生されることを確認

**期待される動作:**
- 録音中はTTS後に約2秒間待機する
- 5件を超える録音は古いものから上書きされる
- ミックス音声は正規化されクリッピングしない

**トラブルシューティング:**
- 録音が開始されない場合  
  - `AudioInputManager`のマイク設定を確認  
  - Consoleにエラーがないか確認  
- ミックスが無音の場合  
  - 録音に成功したか（コンソールログ）を確認  
  - `Resources`に指定した参照クリップが存在するか確認
